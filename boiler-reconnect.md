...
4. Open **Smart Life** and look for the device named **Electric**.
5. Open Device Information and verify the Virtual ID `20415515a4e57cabb4b8` and MAC `a4:e5:7c:ab:b4:b8`.
6. If the controller is online in Deco but missing from Smart Life, treat this as an app/account pairing issue rather than a Wi‑Fi issue.
7. If `ESP_ABB4B8` is not present in Deco, reconnect/pair the controller to the 2.4 GHz network.

## Still missing

- Exact controller hardware/model number.
- Exact pairing/reset procedure for this controller.
- Which Smart Life account/login currently owns the device, if account recovery is ever needed.
